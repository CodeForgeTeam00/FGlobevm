...
const FAQItem: React.FC<FAQItemProps> = ({ number, question, answer, isOpen, onClick }) => {
    return (
        <div
            className={`bg-white rounded-3xl overflow-hidden transition-all duration-500 border ${isOpen ? 'border-sky-300 shadow-lg' : 'border-transparent shadow-sm'}`}
        >
            <button
                onClick={onClick}
                className="w-full text-left p-6 md:p-8 flex items-center justify-between gap-4 group"
            >
        <span className={`text-[15px] md:text-lg font-bold transition-colors ${isOpen ? 'text-primary-6' : 'text-neutral-black'}`}>
          {number}. {question}
        </span>
                <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-all ${isOpen ? 'bg-primary-6 text-white' : 'bg-slate-50 text-slate-400 border border-slate-100'}`}>
